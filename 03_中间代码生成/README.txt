...
*要使目标代码正确执行，由上面的三地址码直接转为的目标代码还是不够的，例如在三地址代码实例2提到的一些问题，另外还有不同作用域中存在同样变量名，我们如何在运行时确定变量的存储单元。所以我们还需要知道程序在内存中的位置、程序执行时的上下文环境、变量的作用域等信息才能生成准确的中间码。这些知识我们将在《04_运行时刻环境》：../04_运行时刻环境/README.txt中学习到